...
- **Upload (v0):** the host puts the file's CRC-32 in the final `CLOSE` DATA
  (4 bytes, big-endian). **v1 extension:** 4 more bytes follow: packed FAT time
  (2, LE) then packed FAT date (2, LE). The agent sets the file's date/time before
  closing the handle. The `CLOSE`-ACK status byte is `0` (OK) or `1` (CRC mismatch).
- **Download (v0):** the agent puts the CRC-32 of what it sent in its `CLOSE`
  (4 bytes, big-endian); the host checks it. **v1 extension:** 4 more bytes: packed
  FAT time (2, LE) then packed FAT date (2, LE). The host applies them to the local
  file with `os.utime`.
...
The host queries the agent's protocol version once at session start with
`T_VERSION` (type 16). An up-to-date agent replies `ACK` with a 1-byte payload =
version (currently `0x01`). An **old agent** falls through to the default empty
`ACK`, which the host maps to **version 0** (no date/time support). All v1
extensions are therefore backward-compatible: a v0 agent works normally, it just
doesn't carry timestamps.

## Packet types

`TYPE` is one byte. Types **1–9** are the frozen base protocol; **10–15** are the
v2 additions (delete/rename/byte-range I/O + raw screen output); **16** is the v3
version query. `ACK`/`NAK` are the agent's replies.
...
| VERSION | query protocol version (v3)        | 16 | — |

### Agent → host

| Type | Name | # | DATA layout |
|------|------|---|-------------|
| ENTRY | one directory entry (reply to LIST) | 8 | v0: `attr`(1) `size`(4, LE) `name`(ASCIIZ, ≤14) · v1: `attr`(1) `size`(4, LE) `time`(2, LE) `date`(2, LE) `name`(ASCIIZ, ≤14) |
...
- `VERSION` → `ACK` with a 1-byte **version** (`0x01` for the current agent). An
  old agent without T_VERSION support returns an empty `ACK`, which the host maps
  to version 0.
...
- **Date/time (v1 only).** FAT packed format: date = `(year-1980)<<9 | month<<5 | day`,
  time = `hours<<11 | minutes<<5 | (seconds/2)`, both 16-bit little-endian. The host
  checks the negotiated version before parsing or sending these extra bytes. A v1
  `ENTRY` is 4 bytes longer than v0; the host skips date/time parsing when talking to
  a v0 agent. Mounted files show real DOS timestamps when connected to a v1 agent.
